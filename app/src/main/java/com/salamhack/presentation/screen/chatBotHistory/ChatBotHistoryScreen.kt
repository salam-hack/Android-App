...
fun ChatBotHistoryScreen(
    viewModel: ChatBotHistoryViewModel = koinViewModel()
){
...
fun ChatBotHistoryScreenContent(
    ...
    action: ChatBotHistoryInteractionListener,
    state: ChatBotHistoryUiState
){
    Box(
        modifier = modifier
            .fillMaxSize()
        ...
        Column(
            ...
            LazyColumn(
                ...
                if (state.isLoading) {
                    item {
                        Text("جاري التحميل...", modifier = Modifier.align(Alignment.CenterHorizontally))
                    }
                }

                if (!state.isLoading && state.displayedChats.isEmpty()) {
                    item {
                        Text(
                            text = state.error ?: "لا يوجد محادثات سابقة",
                ...
                items(
                    count = state.displayedChats.size,
                    key = { index -> state.displayedChats[index].id }
                ) { index ->
                    val chat = state.displayedChats[index]
                    ...
                    ChatCard(
                        ...
                        time = chat.displayTime,
                        modifier = Modifier.clickable {
                            action.onClickChat(chat.id)
                        }
                    )
                }
            }
